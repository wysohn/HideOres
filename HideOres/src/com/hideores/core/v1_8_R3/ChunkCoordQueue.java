    ...
    public boolean isEmpty() {
            ...
            while (iterator1.hasNext() && arraylist.size() < 10) {
                ...
                if (chunkcoordintpair != null) {
                    if (player.getHandle().world.isLoaded(new BlockPosition(chunkcoordintpair.x << 4, 0, chunkcoordintpair.z << 4))) {
            ...
            if (!arraylist.isEmpty()) {
                if (arraylist.size() == 1) {
                	PacketPlayOutMapChunk pomcPacket = new PacketPlayOutMapChunk((Chunk) arraylist.get(0), true, '\uffff');
                	
                	//modify packet before send
                	MapChunkCalculation.calcAndChange(player.getWorld(), pomcPacket);
                	//Bukkit.getLogger().info("sending MapChunk");
                	player.getHandle().playerConnection.sendPacket(pomcPacket);
                } else {
                	PacketPlayOutMapChunkBulk pomcbPacket = new PacketPlayOutMapChunkBulk(arraylist);
                	
                	//modify packet before send
                	MapChunkCalculation.calcAndChange(player.getWorld(), pomcbPacket);
                	//Bukkit.getLogger().info("sending MapChunkBulk");
                ...
                while (iterator2.hasNext()) {
                    ...
                    player.getHandle().u().getTracker().a(player.getHandle(), chunk);
                }
            }
        return true;
    }
